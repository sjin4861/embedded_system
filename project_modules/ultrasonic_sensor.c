...
void RCC_Configure(void) {
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM2, ENABLE);  // TIM2 클럭 활성화
}
...
void TIM2_Configure(void) {
    ...
    TIM_TimeBaseStructure.TIM_Prescaler = 72 - 1; // 1MHz (1µs 단위)
    TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
    TIM_TimeBaseStructure.TIM_Period = 0xFFFF; // 최대 카운터 값
    TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;
    TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure);

    TIM_Cmd(TIM2, ENABLE); // 타이머 활성화
}
...
float Measure_Distance(void) {
    uint32_t start_time = 0, stop_time = 0, echo_time = 0;
    float distance = 0.0;
